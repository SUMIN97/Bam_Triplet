...
class TripletDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for path in self.imgs_path:
            material = path.split('/')[-1].split('_')[0]
            author = path.split('/')[-1].split('_')[1]
            if material == anchor_material or author == anchor_author:
                potivie_imgs.append(self.read_image(path))
            else:
                negative_imgs.append(self.read_image(path))
        ...
        return anchor, positive_imgs, negative_imgs
...
for epoch in range(n_epochs):
    ...
    correct = 0
    for batch_idx, (anchor, positive_imgs, negative_imgs) in enumerate(tqdm(loader)):
        torch.cuda.empty_cache()
        if use_cuda:
        ...
        random.shuffle(positive_imgs)
        random.shuffle(negative_imgs)

        final_index = np.min(len(positive_imgs), len(negative_imgs))
        for i in range(final_index+1):
            positive = torch.cat((style_model(positive_imgs[i]).double(), content_model(positive_imgs[i]).double()), dim=1)
            negative = torch.cat((style_model(negative_imgs[i]).double(), content_model(negative_imgs[i]).double()), dim=1)
